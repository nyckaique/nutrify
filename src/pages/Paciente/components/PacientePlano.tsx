...
export default function PacientePlano({ p }: PacienteInfoProps) {
  ...
  async function formSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (file !== null) {
      const uploadRef = ref(storage, `planos/${paciente!.id}/${file.name}`);
      uploadBytes(uploadRef, file).then((snapshot) => {
        getDownloadURL(snapshot.ref).then(async (downloadUrl) => {
          ...
          await updateDoc(docRef, {
            ...paciente,
            planos: [...paciente!.planos, { fileName, urlFile, data }],
          })
            .then(() => {
              toast.success("Adicionado plano alimentar com sucesso");
  ...
  function handleExcluirFile(fileIndex: number) {
    const fileParaExcluir = p.planos[fileIndex!];
  ...
  function handleConfirmarExclusao() {
    excluirPlano(indexFile!, `planos/${paciente!.id}/${fileAtual!.fileName}`);
    setModalDeleteVisivel(false);
  }
  const filesComIndice = p.planos.map((plano, index) => ({
    ...plano,
    originalIndex: index,
  }));
    ...
    <div>
      <div className="p-3 shadowblack rounded-lg my-3 border-zinc-200 border-[1px] flex-1 overflow-hidden flex flex-col gap-3 ">
        ...
        {filesOrdenadas.length > 0 &&
          filesOrdenadas.map((plano, indexFiles) => (
            <div key={indexFiles}>
              <button
                onClick={() => handleExcluirFile(plano.originalIndex)}
      ...
      {modalDeleteVisivel && (
        <div className="modal">
          <div className="modal-content w-[400px]">
            ...
            <p className="mb-2">
              Tem certeza que deseja excluir essa plano alimentar?
            </p>
